...
$ffmpegTextBoxSettings.Text = "-copy_unknown -map_metadata 0 -map 0:0 -map 0:1 -c:v libx265 -crf 23 -c:a aac -b:a 256k -c:s mov_text -threads 0"
...
$ffmpegDeNoiseTextBoxSettings.Text = "nlmeans=`"1.0:7:5:3:3`""
...
foreach ($file in $mp4Files) {
  ...
  # Set the ffmpeg arguments
  $arguments = "-i `"$inputFile`" -vf `"$ffmpegStab2Settings,$ffmpegDeNoiseSettings`" $ffmpegSettings `"$outputFile`""
  Write-Output "$arguments"
  $arguments = $arguments.Split(" ")
  ...
  # Set the ExifTool arguments
  $arguments = "-ee -overwrite_original `"-filemodifydate<datetimeoriginal`" `"-filecreatedate<datetimeoriginal`" `"$outputFile`""
  Write-Output "$arguments"
  $arguments = $arguments.Split(" ")
